...
contract('RealEstate', function (accounts) {
    ...
    it("가나슈 두번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function () {
        return RealEstate.deployed().then(function (instance) {
            ...
            assert.equal(recipt.logs.length, 1, "이벤트 하나가 생성되지 않았습니다.");
            assert.equal(recipt.logs[0].event, "LogBuyRealEstate", "이벤트가 LogBuyRealEstate가 아닙니다.");
            assert.equal(recipt.logs[0].args._buyer, accounts[1], "매입자가 가나슈 두번째 계정이 아닙니다.");
            assert.equal(recipt.logs[0].args._id, 0, "매물 아이디가 0이 아닙니다.");
            ...
            assert.equal(buyers[0].toUpperCase(), accounts[1].toUpperCase(), "Buyers 배열 첫 번째 인덱스의 계정이 가나슈 두번째 계정과 일치하지 않습니다.");
        })
    })
});